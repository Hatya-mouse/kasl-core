...
fn test_process_simple_buffer_shader() {
    ...
    let process_result = node.process(48000, 24000.0, 1, 0, 3, 0);
    assert!(
        process_result.is_ok(),
        "Process failed: {:?}",
        process_result.err()
...
fn test_get_output_before_and_after_process() {
    ...
    assert!(node.process(48000, 24000.0, 1, 0, 1, 0).is_ok());
...
fn test_clone_node() {
    ...
    assert!(original_node.process(48000, 24000.0, 1, 0, 1, 0).is_ok());
...
fn test_process_with_no_program_should_panic_or_err() {
    ...
    let result = std::panic::catch_unwind(move || node.process(48000, 24000.0, 1, 0, 1, 0));
    assert!(
        result.is_err(),
