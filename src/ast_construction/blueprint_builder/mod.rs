...
impl<'a> BlueprintBuilder<'a> {
    pub fn new(comp_state: &'a CompilationState) -> Self {
        Self { comp_state }
    }

    pub fn build(&self) -> IOBlueprint {
        let mut blueprint = IOBlueprint::default();
        let global_scope = self.comp_state.scope_registry.get_global_scope();

        // Loop over each variables in the global scope
        for var_id in &global_scope.variables {
            let Some(scope_var) = self.comp_state.scope_registry.get_var_by_id(var_id) else {
                continue;
            };
